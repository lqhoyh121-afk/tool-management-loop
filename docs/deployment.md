...
**「处理」也逐条点名（#93）**：摘要给计数（`处理 N`），下面每条处理各有一行 `处理 <kind> loan=… <source_kind>=… [动作] 迁移`，例如 `处理 event loan=<单号> todo=<待办 id> confirm_issue awaiting_issue_confirmation→borrowed`。`processed` 与这些行**一一对应**（条数永远相等），迁移是这一条的净效果（审批那一条落了两笔写也只报一段）。已消费的**阶段待办完成事件**就在这一桶里，它是「点待办即证据」唯一的正向记录：真机实测一条已完成的【待领用确认】被正常消费、台账推进到已借出，而报告里除「处理 1」之外一个字都没有 —— 跳过行只印跳过的，于是这次正常推进被读成了「引擎漏消费、事件根本没进扫描面」。要回答「引擎到底看见我点的那条待办没有」，看这一行，不要靠数数。没得处理时一行都不印（有行就等于说推进了单）。
