...
无人值守要求申请行能自己变成台账行，所以表单上必须有两道**指向**题，缺了就只能靠操作员登记（见「申请自动发现」）：

- **物品指向**：指向工器具台账/库存表的关联记录题（或写「物品所在表 + 该表记录 id」的两格）。对应绑定里的 `apply_fields.item_container` / `apply_fields.item_id`。
